    ...
    <table class="table table-bordered " id="myTable">
      ...
      <tbody>
        ...
        <tr>
           ...
           <td>{{$usr->email}}</td>
           <td>{{$usr->no_telp}}</td>
           <td>{{$usr->jalan}}, {{$usr->desa}}, {{$usr->kecamatan}}, {{$usr->kota}} </td>
           <td>
             <button type="button" class="btn {{$usr->status ? 'btn-success' : 'btn-danger'}} btn-sm">{{$usr->status ? 'Aktif' : 'Tidak aktif'}}</button>
           </td>
           <td>
             <a data-toggle="modal" data-target=".bs-modal-md-info{{$usr->id}}" class="mr-3 btn btn-sm {{$usr->status ? 'btn-danger' : 'btn-success'}}">{{$usr->status ? 'Non Aktifkan User' : 'Aktifkan User'}}</a>
           </td>
